...
export class TextmateLanguageGrammarContribution extends Disposable implements ClientAppContribution {
  ...
  async initialize() {
    languages.map(languageId => getLanguageById(languageId)).filter(item => !!item).forEach((lang) => {
      try {
        import(/* webpackChunkName: "opensumi-textmate-languages" */ `@opensumi/textmate-languages/es/${lang!.extensionPackageName}`)
    ...
    vueSFC(this.textMateService.registerLanguage.bind(this.textMateService),
      this.textMateService.registerGrammar.bind(this.textMateService))
  }
}
